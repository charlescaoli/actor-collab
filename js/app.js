...
async function selectActor(person) {
  ...
  try {
    ...
    bannerTopMovies.innerHTML = top5.map(w => `
      <div>
        ${w.poster_path
          ? `<img class="top-movie-thumb" src="${posterUrl(w.poster_path, 'w92')}" alt="${esc(w.title)}" title="${esc(w.title)}" loading="lazy" data-movie-type="${w.type}" data-movie-id="${w.id}">`
          : `<div class="top-movie-thumb" style="background:rgba(255,255,255,0.04);display:flex;align-items:center;justify-content:center;font-size:0.6rem;color:#5a5a6e;cursor:pointer" data-movie-type="${w.type}" data-movie-id="${w.id}" title="${esc(w.title)}">${esc(w.title)}</div>`
        }
        <div class="top-movie-label">${esc(w.title)}</div>
      </div>
    `).join('');
